...
export async function GET(request: Request) {
  ...
  // Try retrieving token from request query or server token route
  const tokenParam = searchParams.get('token');
  let token = tokenParam;

  if (!token) {
    try {
      const tokenRes = await fetch(new URL('/api/ml-token', request.url).toString());
      if (tokenRes.ok) {
  ...
  // Fallback to rich mock sales data if no token or API call fails
  const mockData = getMockOrders();
  return NextResponse.json({
    source: 'mock',
    orders: mockData,
    total: mockData.length,
    message: token ? 'Live API request failed; displaying read-only mock dataset.' : 'No access token provided; displaying read-only demo sales data.'
  });
}
